# Fix for Supabase Database Setup Error

## The Problem
You got: `ERROR: 42703: column "status" does not exist`

This means the profiles table structure is different than expected.
...
Run this to safely add the missing columns:
...
### Step 3: Create the CMS Tables
Now run this to create all the CMS tables:
...
### Step 6: Add Triggers and Sample Data
```sql
...
After creating your account at http://localhost:5173/Login, run:
...
SELECT id, email, role, has_paid FROM profiles WHERE role = 'admin';

-- Check tables exist
SELECT table_name FROM information_schema.tables 
WHERE table_schema = 'public' 
AND (table_name LIKE 'blog_%' OR table_name LIKE 'education_%' OR table_name LIKE 'product_%' OR table_name = 'media_library');

-- Check sample data
SELECT title, status FROM blog_posts;
```

That should fix the database setup completely!